pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/rals-dev/rs-field-service.git'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@git@github.com:rals-dev/rs-field-service.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "/home/ralsaditya/rals-soccer/rs-field-service"
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:rals-dev/rs-field-service.git "${targetDir}"
                cd "${targetDir}"
            fi
